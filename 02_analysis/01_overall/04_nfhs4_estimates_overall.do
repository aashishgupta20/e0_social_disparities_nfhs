...
**************************************************
	...
	*Log
	cap log close
	log using "$dir/02_logs/nfhs4_overall_ci.txt", text replace
...
********************************************************
	...
	*ex se 
	...
	save "$dir\04_input\resamples\bstraps\nfhs4\nfhs4_ex_se_overall.dta", replace
	...
	*nmx se 
	...
	save "$dir\04_input\resamples\bstraps\nfhs4\nfhs4_nmx_se_overall.dta", replace
...
*********************************************************
	...
	*Keep only ex's
	...
	"$dir\04_input\resamples\bstraps\nfhs4\nfhs4_ex_se_overall.dta", nogen

	gen ex_lci = ex - 1.96*se_ex
	gen ex_uci = ex + 1.96*se_ex
	
	merge 1:1 female age_group using ///
	"$dir\04_input\resamples\bstraps\nfhs4\nfhs4_nmx_se_overall.dta", nogen 
...
*******************************************************
	...
	saveold "$dir/05_out/estimates/nfhs4_overall_life_tables_se.dta", replace 
...
***********************************************************
	...
	*male 
	...
	;
	# d cr
	graph save "$dir/05_out/figures/srs_nfhs4_nmx_male.gph", replace
	...
	*female
	...
	;
	# d cr
	graph save "$dir/05_out/figures/srs_nfhs4_nmx_female.gph", replace
	
	*combine
	graph combine /// 
	"$dir/05_out/figures/srs_nfhs4_nmx_female.gph" ///
	"$dir/05_out/figures/srs_nfhs4_nmx_male.gph", ///
	row(1) ///
	xsize(4) ysize(1.4) ///
	graphregion(fcolor(white) lcolor(white)) 
	graph save "$dir/05_out/figures/srs_nfhs4_nmx.gph", replace
	graph export "$dir/05_out/figures/srs_nfhs4_nmx.pdf", replace
