...
**********************************************************************
	...
	graph twoway ///
	(connected survivors age_group if female == `sex' & round == `round' & caste_religion==1, ///
		msymbol(Dh) msize(small) lcolor(navy) mcolor(navy) lpattern(dash_dot)) ///
	(connected survivors age_group if female == `sex' & round == `round' & caste_religion==2, ///
		msymbol(Sh)  msize(small) lcolor(sienna) mcolor(sienna) lpattern(dash)) ///
	(connected survivors age_group if female == `sex' & round == `round' & caste_religion==2, ///
		msymbol(Sh)  msize(small) lcolor(sienna) mcolor(sienna) lpattern(dash)) ///
	(connected survivors age_group if female == `sex' & round == `round' & caste_religion==4, ///
		msymbol(Dh)  msize(small) lcolor(forest_green) mcolor(forest_green) lpattern(longdash)) ///
	(connected survivors age_group if female == `sex' & round == `round' & caste_religion==5, ///
		msymbol(Th)  msize(small) lcolor(dkorange) mcolor(dkorange) lpattern(longdash_dot)) ///
	(rarea survivors_uci survivors_lci age_group  if female == `sex' & round == `round' & caste_religion==1, ///
		color(navy%20) lwidth(none)) ///
	(rarea survivors_uci survivors_lci age_group if female == `sex' & round == `round' & caste_religion==2, ///
		color(maroon%20) lwidth(none)) ///
	(rarea survivors_uci survivors_lci age_group if female == `sex' & round == `round' & caste_religion==4, ///
		color(forest_green%20) lwidth(none)) ///
	(rarea survivors_uci survivors_lci age_group if female == `sex' & round == `round' & caste_religion==5, ///
		color(dkorange%20) lwidth(none))  ///	
	, ///
	graphregion(fcolor(white) lcolor(white)) ///
	legend(order(1 "Scheduled Caste" 2 "Scheduled Tribes" 3 "Other Backward Classes" 4 "High Caste") ///
...
*nmx 
	...
	(connected ln_nmx age_group if female == `sex' & round == `round' & caste_religion==1, ///
		msymbol(Oh) msize(small) lcolor(navy) mcolor(navy) lpattern(dash_dot)) ///
	(connected ln_nmx age_group if female == `sex' & round == `round' & caste_religion==2, ///
		msymbol(Sh)  msize(small) lcolor(maroon) mcolor(maroon) lpattern(dash)) ///
	...
	graph save "$dir\05_out\figures\nmx_group_round`round'_male`sex'.gph", replace
	...
	grc1leg ///
	"$dir\05_out\figures\nmx_group_round2_male0.gph" ///
	"$dir\05_out\figures\nmx_group_round2_male1.gph" ///
	"$dir\05_out\figures\nmx_group_round4_male0.gph" ///
	"$dir\05_out\figures\nmx_group_round4_male1.gph" ///
	, ///
	graphregion(fcolor(white) lcolor(white)) ///
