...
********************************************************	
	...
	*women 
	use "$dir/05_out/estimates/nfhs4_group_region_life_tables_se.dta", clear  
		...
		*keeps and drops
		keep if age_group==0
		keep if caste_religion == 1 | caste_religion == 5 
		keep if female==1 
		...
		*save
		saveold "$dir\03_intermediate\region_ex_reshape_women.dta", replace
	...
	*men 
	use "$dir/05_out/estimates/nfhs4_group_region_life_tables_se.dta", clear  
		...
		*save 
		saveold "$dir\03_intermediate\region_ex_reshape_men.dta", replace
	
	
	*labels 
	
		*women: sc v hc 
		use "$dir\03_intermediate\region_ex_reshape_women.dta", clear 
		gen labtype = 1 
		
		append using "$dir\03_intermediate\region_ex_reshape_women.dta"
		...
		*generate lables 
		gen ex_label = "SC e{sub:0}: " if labtype == 1
		replace ex_label = string(ex_sc, "%6.1f") if labtype == 2 
		replace ex_label = "[" + string(ex_lci1, "%3.1f") + "-" + string(ex_uci1, "%3.1f") + "]"	if labtype == 3
		...
		*region name adjustments 
		...
		save "$dir/03_intermediate/nfhs4_women_lables_sc.dta", replace 
		
	*men: sc v hc 
	
		*append to creare 6 labels 
		use "$dir\03_intermediate\region_ex_reshape_men.dta", clear 
		gen labtype = 1 
		
		append using "$dir\03_intermediate\region_ex_reshape_men.dta"
		replace labtype = 2 if labtype == . 
		...
		*adjust label positions 
			...
			*save 
			save  "$dir/03_intermediate/nfhs4_men_lables_sc.dta", replace 
...
*******************************************************************************
	...
	*women, sc v hc 
	...
	spmap ///
		...
		ocolor(white ..) ///
		osize(medthin ..) ///	
		label(data("$dir/03_intermediate/nfhs4_women_lables_sc.dta") ///
			xcoord(x_stub)  ///
			ycoord(y_stub) ///
			label(ex_label) ///
			by(labtype) ///
		...
		subtitle("a) SC and HC women", size(*1.2)) name(women)
		graph save "$dir/05_out/figures/nfhs4_sc_hc_women_map.gph", replace
	...
	*men, sc v hc 
	...
	spmap ///
		...
		ocolor(white ..) ///
		osize(medthin ..) ///	
		label(data("$dir/03_intermediate/nfhs4_men_lables_sc.dta") ///
			xcoord(x_stub)  ///
			ycoord(y_stub) ///
			label(ex_label) ///
			by(labtype) ///
			size(*1.1 *1 *0.75 *1.1 *1 *0.75 *1 ..) ///
			color(black black gs4 black black gs4 navy ..)) ///
		subtitle("b) SC and HC men", size(*1.2)) name(men)
		graph save "$dir/05_out/figures/nfhs4_sc_hc_men_map.gph", replace
